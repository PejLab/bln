...
plot(x = r.approx, y = matlab$px)
```

```{r cpp_approx}
system.time(cpp.approx <- dblnpp(x = matlab$x, size = matlab$x + matlab$xc, mean = matlab$mu, sd = sqrt(x = matlab$v)))
plot(x = cpp.approx, y = matlab$px)
```
...
plot(x = r.exact, y = matlab$px_acc)
```
